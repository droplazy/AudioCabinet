...
static void bt_test_bond_state_cb(btmg_bond_state_t state, const char *bd_addr)
{
    ...
    if (state == BTMG_BOND_STATE_BONDED) {
        ...
        BTMG_INFO("Pairing state for %s is BONDING", bd_addr);
    }
}
...
void bt_test_agent_request_passkey_cb(void *handle, char *device)
{
    ...
    BTMG_INFO("AGENT: request passkey, please enter passkey:");
    scanf("%06d", &passkey);
    BTMG_INFO("input passkey:%06d", passkey);
...
static void bt_test_a2dp_sink_audio_stream_cb(const char *bd_addr, uint16_t channels, uint32_t sampling,
                                       snd_pcm_format_t format, uint8_t *data, uint32_t len)
{
    BTMG_INFO("sam:%d chan:%d fm:%d len:%d", sampling, channels, format, len);
    /*TODO*/
}
...
static void bt_test_hfp_hf_connection_state_cb(btmg_hfp_hf_connection_state_t state)
{
    if (state == BTMG_HFP_HF_DISCONNECTED) {
        BTMG_INFO("hfp hf disconnected");
    } else if (state == BTMG_HFP_HF_CONNECTED) {
        BTMG_INFO("hfp hf connected");
    } else if (state == BTMG_HFP_HF_CONNECT_FAILED) {
        BTMG_INFO("hfp hf connect failed!");
    } else if (state == BTMG_HFP_HF_DISCONNEC_FAILED) {
        BTMG_INFO("hfp hf disconnect failed!");
    } else if (state == BTMG_HFP_HF_SCO_LINK_CONNECTED) {
        BTMG_INFO("hfp hf sco link connected!");
    } else if (state == BTMG_HFP_HF_SCO_LINK_DISCONNECTED) {
        BTMG_INFO("hfp hf sco link disconnected!");
    }
}

static void bt_test_hfp_ag_connection_state_cb(btmg_hfp_ag_connection_state_t state)
{
    if (state == BTMG_HFP_AG_DISCONNECTED) {
        BTMG_INFO("hfp ag disconnected");
    } else if (state == BTMG_HFP_AG_CONNECTED) {
        BTMG_INFO("hfp ag connected");
    } else if (state == BTMG_HFP_AG_CONNECT_FAILED) {
        BTMG_INFO("hfp ag connect failed!");
    } else if (state == BTMG_HFP_AG_DISCONNEC_FAILED) {
        BTMG_INFO("hfp ag disconnect failed!");
    } else if (state == BTMG_HFP_AG_SCO_LINK_CONNECTED) {
        BTMG_INFO("hfp ag sco link connected!");
    } else if (state == BTMG_HFP_AG_SCO_LINK_DISCONNECTED) {
        BTMG_INFO("hfp ag sco link disconnected!");
    }
}

static void bt_test_hfp_at_event_cb(struct btmg_at_t at_msg)
{
    static const char *types[BTMG_AT_TYPE_MAX] = {
        [BTMG_AT_TYPE_RAW] = "RAW",     [BTMG_AT_TYPE_CMD] = "CMD",       [BTMG_AT_TYPE_CMD_GET] = "GET",
        [BTMG_AT_TYPE_CMD_SET] = "SET", [BTMG_AT_TYPE_CMD_TEST] = "TEST", [BTMG_AT_TYPE_RESP] = "RESP",
    };
    BTMG_INFO("AT message: %s: command:%s, value:%s",
            types[at_msg.type], at_msg.command, at_msg.value);
}
...
static int _bt_init(int profile)
{
    ...
    if (bt_pro_info->is_a2dp_sink_enabled) {
        ...
        /* sink pcm stream callback*/
        // bt_callback->btmg_a2dp_sink_cb.a2dp_sink_stream_cb = bt_test_a2dp_sink_audio_stream_cb;
        // bt_manager_a2dp_sink_stream_cb_enable(true);
    }
    ...
    if (bt_pro_info->is_hfp_hf_enabled) {
        bt_callback->btmg_hfp_cb.hfp_hf_connection_state_cb = bt_test_hfp_hf_connection_state_cb;
        bt_callback->btmg_hfp_cb.hfp_at_event_cb = bt_test_hfp_at_event_cb;
    }
    /* bt hfp ag callback*/
    if (bt_pro_info->is_hfp_ag_enabled) {
        bt_callback->btmg_hfp_cb.hfp_ag_connection_state_cb = bt_test_hfp_ag_connection_state_cb;
        bt_callback->btmg_hfp_cb.hfp_at_event_cb = bt_test_hfp_at_event_cb;
    }
    /* bt spp callback*/
    ...
    add_cmd_table(common_cmd_table);

    /*BT*/
    if (bt_pro_info->is_a2dp_sink_enabled || bt_pro_info->is_a2dp_source_enabled ||
        bt_pro_info->is_avrcp_enabled || bt_pro_info->is_hfp_hf_enabled ||
...
static int kill_test_process(void)
{
    ...
    sprintf(cmd, "ps | grep bt_test | grep -v grep | grep -v %d | grep -v bin | awk '{print $1}' | xargs",
            pid_self);
    strea = popen(cmd, "r");
...
void cmd_table_init(void)
{
    ...
    cmd_tbl_t * new_list;
    /*default*/
    for (s_index; common_cmd_table[table_number].cmd;) {
        memcpy(g_cmd_list + s_index, &(common_cmd_table[table_number]), sizeof(cmd_tbl_t));
        s_index++;
        table_number++;
