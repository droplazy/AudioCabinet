...
static struct pcm_config *a2dp_src_pcm_pf = NULL;

static struct pcm_config a2dp_src_pcm = {
    .device = "bluealsa:DEV=00:00:00:00:00:00",
    .pcm = NULL,
...
static int _a2dp_src_pcm_write(void *handle, char *buff, uint32_t *len)
{
    ...
    ex_frames = *len / (cf->bit_p_spl / 8 * cf->channels);
    residue = ex_frames;
    total = ex_frames;
...
static int _a2dp_src_pcm_deinit(void *usr_data)
{
    ...
    if (a2dp_src_pcm_pf && a2dp_src_pcm_pf->pcm != NULL) {
...
int bt_a2dp_src_stream_send(char *data, int len)
{
    ...
    byte_s = a2dp_src_pcm_pf->sampling * a2dp_src_pcm_pf->channels * a2dp_src_pcm_pf->bit_p_spl / 8;
...
int bt_a2dp_src_init(uint16_t channels, uint32_t sampling, int format)
{
    ...
    a2dp_src_pcm_pf = &a2dp_src_pcm;

    a2dp_src_pcm_pf->buffer_time = 400000;
    a2dp_src_pcm_pf->period_time = 100000;
    a2dp_src_pcm_pf->format = format;
    a2dp_src_pcm_pf->bit_p_spl = snd_pcm_format_physical_width((snd_pcm_format_t)format);
    ...
    if (transmit_init(a2dp_src_ts, byte_s) == 0) {
        ...
        a2dp_src_ts->comsumer.com.sem.app_cb_deinit = _a2dp_src_pcm_deinit;
