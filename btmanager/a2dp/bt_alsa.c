...
static int pcm_open(struct pcm_config *p, char **msg)
{
    ...
    p->pcm = _pcm;
    /* T527 alsa-lib-1.26 set_period_time_near err */
    /*
    ...
    */
    if ((err = snd_pcm_set_params(_pcm,
                        p->format,
                        ...
                        1,
                        4 * p->period_time)) < 0) {
        LG_ALSA_ERROR("Playback open error: %s\n", snd_strerror(err));
        goto fail;
    }

    //reset threadshold for hfp capture
    if (p->stream == SND_PCM_STREAM_CAPTURE) {
        if ((err = snd_pcm_get_params(_pcm, &buffer_size, &period_size)) != 0) {
            snprintf(buf, sizeof(buf), "Get params: %s", snd_strerror(err));
            goto fail;
        }
...
void aw_pcm_free(struct pcm_config *pf)
{
    ...
    BTMG_INFO("-->free pcm(%s),address:%p,stream:%s,drop:%d", pf->device, pf->pcm,
               pf->stream ? "captrue" : "playback", pf->drop);
    ...
    if (pf->pcm != NULL) {
        ...
        BTMG_INFO("<--snd pcm closed:%s", pf->device);
        pf->pcm = NULL;
    }
}
...
int aw_pcm_open(struct pcm_config *pf)
{
    ...
    if (pf->pcm == NULL) {
        ...
        LG_ALSA_DEBUG("\n \
                    ...
                    Sampling rate: %u Hz\n \
                    Channels:%u",
                      pf->device, pf->buffer_time, pf->period_time,
