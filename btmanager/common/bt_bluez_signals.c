...
* Author: BT Team
* Date: 2022.03.12
* Description: bluez signal handle.
*/
...
extern bool get_gatts_connect_status(void);
...
static void _mediaplayer_status(GVariant *value, const gchar *path)
{
    ...
    BTMG_DEBUG("player status: %s", player_status);
                                                         ...
                                                         void *userdata)
{
    ...
    /*fix no a2dp disconnect callback when ble server connected*/
    if (!is_mediacontrol_connected && (get_gatts_connect_status() == true)) {
        BTMG_DEBUG("special handle");
        dev_node = btmg_dev_list_find_device(connected_devices, remote_address);
        if (dev_node == NULL) {
            goto fail;
        }
        ...
        if (bt_pro_info->is_a2dp_sink_enabled) {
            BTMG_DEBUG("a2dp sink disconnected,device:%s", remote_address);
            if (btmg_cb_p && btmg_cb_p->btmg_a2dp_sink_cb.a2dp_sink_connection_state_cb)
                btmg_cb_p->btmg_a2dp_sink_cb.a2dp_sink_connection_state_cb(
                        remote_address, BTMG_A2DP_SINK_DISCONNECTED);
        }
        if (bt_pro_info->is_a2dp_source_enabled) {
            BTMG_DEBUG("a2dp source disconnected,device:%s", remote_address);
            if (btmg_cb_p && btmg_cb_p->btmg_a2dp_source_cb.a2dp_source_connection_state_cb)
                btmg_cb_p->btmg_a2dp_source_cb.a2dp_source_connection_state_cb(
                        remote_address, BTMG_A2DP_SOURCE_DISCONNECTED);
        }
        bt_device_set_state(BTMG_DEVICE_STATE_DISCONNECTED);
        BTMG_DEBUG("remove device %s from connected_devices", remote_address);
        btmg_dev_list_remove_device(connected_devices, remote_address);
    }
                                                           ...
                                                           void *userdata)
{
    ...
    while (g_variant_iter_next(properties, "{&sv}", &key, &value)) {
        if (!strcmp(key, "Volume")) {
            ...
            if (btmg_cb_p->btmg_avrcp_cb.avrcp_audio_volume_cb)
                btmg_cb_p->btmg_avrcp_cb.avrcp_audio_volume_cb(btmg_path_to_addr(path), volumes);
        } else if (!strcmp(key, "State")) {
            strcpy(state, g_variant_get_string(value, NULL));
            BTMG_INFO("transport state is :%s", state);
