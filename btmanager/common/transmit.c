...
* Author: BT Team
* Date: 2022.03.12
* Description: btmanager transmit function.
*/
...
int transmit_sem_post(ts_sem_t *s, bool wait_enable)
{
    ...
    TS_LOG_INFO("sem wait_enable %d", wait_enable);
...
int transmit_sem_wait(ts_sem_t *s)
{
    ...
    pthread_mutex_lock(&s->mutex);
    if (s->wait_enable == true) {
        TS_LOG_DEBUG("into wait...");
        if (s->app_cb_deinit && (card_close_flag == 1)) {
            TS_LOG_DEBUG("close card...");
            card_close_flag = 0;
            s->app_cb_deinit(NULL);
        }
...
static void *comsumer_thread(void *arg)
{
    ...
    while (co->thread_loop) {
        ...
        if (c->cache_enable) {
            ...
            TS_LOG_WARN("fifo is enough:(%d-%d)", ret, c->cache);
        }
        ...
        if (len == -1) {
            ...
            c->cache_enable = true;
            TS_LOG_WARN("fifo is empty %d", ring_buff_data_len(&t->rb));
            continue;
        }
        pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);
        pthread_mutex_lock(&co->mutex);
