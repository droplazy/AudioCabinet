...
* Author: BT Team
* Date: 2022.03.12
* Description: dbus agent function.
*/
...
int bt_agent_register(btmg_io_capability_t io_cap)
{
    ...
    result = g_dbus_connection_call_sync(bluez_mg.dbus, "org.bluez", "/org/bluez", "org.bluez.AgentManager1",
                                "RegisterAgent",
...
int bt_agent_set_default(void)
{
    ...
    BTMG_DEBUG("set default agen success");
exit:
    if (result != NULL)
        g_variant_unref(result);
