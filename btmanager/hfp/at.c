...
* Author: BT Team
* Date: 2022.03.12
* Description: AT message function about hfp.
*/
...
{
    switch (type) {
    case BTMG_AT_TYPE_RAW:
        snprintf(buffer, size, "%s", command);
        break;
    case BTMG_AT_TYPE_CMD:
        snprintf(buffer, size, "AT%s\r", command);
        break;
    case BTMG_AT_TYPE_CMD_GET:
        snprintf(buffer, size, "AT%s?\r", command);
        break;
    case BTMG_AT_TYPE_CMD_SET:
        snprintf(buffer, size, "AT%s=%s\r", command, value);
        break;
    case BTMG_AT_TYPE_CMD_TEST:
        snprintf(buffer, size, "AT%s=?\r", command);
        break;
    case BTMG_AT_TYPE_RESP:
        if (command != NULL)
            snprintf(buffer, size, "\r\n%s:%s\r\n", command, value);
        else
            snprintf(buffer, size, "\r\n%s\r\n", value);
        break;
    case BTMG_AT_TYPE_MAX:
        break;
    }
    return buffer;
}
...
char *at_parse(const char *str, struct btmg_at_t *at)
{
    ...
    if (is_command) {
        ...
        if ((tmp = strchr(command, '=')) != NULL) {
            if (tmp[1] == '?')
                at->type = BTMG_AT_TYPE_CMD_TEST;
            else {
                at->type = BTMG_AT_TYPE_CMD_SET;
                at->value = tmp + 1;
            }
        } else if ((tmp = strchr(command, '?')) != NULL)
            at->type = BTMG_AT_TYPE_CMD_GET;
        else
            at->type = BTMG_AT_TYPE_CMD;

        if (tmp != NULL)
            *tmp = '\0';

    } else {
        at->type = BTMG_AT_TYPE_RESP;
    ...
    // BTMG_DEBUG("AT message: %s: command: %s, value: %s", at_type2str(at->type), at->command,
    //           at->value);
