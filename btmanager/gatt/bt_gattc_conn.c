...
* Author: BT Team
* Date: 2022.03.12
* Description: gatt client connect function.
*/
...
re_conn:
    ...
    if (connect(sock, (struct sockaddr *)&dstaddr, sizeof(dstaddr)) < 0) {
        ...
        BTMG_ERROR("Failed to connect: %s", strerror(errno));
        close(sock);
        if((t2.tv_sec - t1.tv_sec) < ATT_RECONN_TIMEOUT && retry_count == 0) {
            usleep(1000 * 1000);
            BTMG_INFO("retry connect");
            retry_count++;
            goto re_conn;
        }
        return -1;
