...
* Author: BT Team
* Date: 2022.03.12
* Description: gatt server function.
*/
...
static int bt_gatts_init(void **call_args, void **cb_args)
{
    ...
    bt_adapter_set_le_host_support();
    RETURN_INT(ret);
}
                                 ...
                                 struct bt_att *att, void *user_data)
{
    ...
    if (opcode == BT_ATT_OP_WRITE_REQ)
        need_rsp = true;
    else if (opcode == BT_ATT_OP_WRITE_CMD || opcode == BT_ATT_OP_PREP_WRITE_REQ || opcode == BT_ATT_OP_EXEC_WRITE_REQ)
        need_rsp = false;
    else
    ...
    if (opcode == BT_ATT_OP_WRITE_CMD || opcode == BT_ATT_OP_PREP_WRITE_REQ || opcode == BT_ATT_OP_EXEC_WRITE_REQ){
...
static int bt_gatts_add_char(void **call_args, void **cb_args)
{
    ...
    BTMG_DEBUG("Permission of characteristic at handle 0x%04x is 0x%08x", char_handle, c->permissions);
...
static int bt_gatts_add_desc(void **call_args, void **cb_args)
{
    ...
    BTMG_DEBUG("Permission of desc at handle 0x%04x is %08x", desc_handle, d->permissions);
...
static int bt_gatts_send_write_rsp(void **call_args, void **cb_args)
{
    ...
    if (!attrib) {
        BTMG_ERROR("couldn't find attribute for handle: 0x%04x", s->attr_handle);
        RETURN_INT(-1);
    }
...
static int bt_gatts_send_rsp(void **call_args, void **cb_args)
{
    ...
    BTMG_DEBUG("send response for handle: 0x%04x,response status: %u", s->attr_handle, s->status);
...
{
    BTMG_DEBUG("enter");
    if(gatt_server == NULL || gatt_server->gatts == NULL) {
        BTMG_WARNG("no conn status");
        RETURN_INT(-1);
    }
