...
* Author: BT Team
* Date: 2022.03.12
* Description: Operate the remote device.
*/
...
int bt_device_connect(const char *addr)
{
    ...
    if (bt_device_get_state() == BTMG_DEVICE_STATE_CONNECTED && _is_connected(path)) {
    ...
    /* pause scan before connect*/
    if (bt_adapter_is_scanning() == true) {
        bt_adapter_stop_scan();
        scan_pre_status = 1;
    }
...
exit:
    ...
    /* recovery scan*/
    if (scan_pre_status == 1) {
        bt_adapter_start_scan();
        scan_pre_status = 0;
    }
...
int bt_remove_unpaired_devices(void)
{
    ...
    for (i = 0; i < count; i++) {
        if (!(bt_device_is_paired(device_list[i].remote_address) | bt_device_is_connected(device_list[i].remote_address))) {
