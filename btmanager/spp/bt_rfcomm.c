...
* Author: BT Team
* Date: 2022.03.12
* Description: rfcomm handle about spp.
*/
...
bt_rfcomm_t *rfcomm = NULL;
static pthread_mutex_t rfcomm_mutex = PTHREAD_MUTEX_INITIALIZER;

int rfcomm_init(void)
{
    rfcomm = (bt_rfcomm_t *)malloc(sizeof(bt_rfcomm_t));
    if (NULL == rfcomm) {
        BTMG_ERROR("malloc for rfcomm failed");
        return BT_ERROR;
    }
    memset(rfcomm, 0, sizeof(bt_rfcomm_t));
...
int rfcomm_deinit(void)
{
    ...
    if (rfcomm->sock > 0) {
        rfcomm_release_dev(rfcomm->sock, rfcomm->dev);
    }
    if (rfcomm) {
        free(rfcomm);
        rfcomm = NULL;
